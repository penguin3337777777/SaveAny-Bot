...
The entire history scan completes before any duplicate checks, downloads or uploads begin. Scanning failure or cancellation never starts saving. The existing matched-video counter becomes the fixed candidate total after scanning; final saved + skipped + failed equals that total on normal completion.
...
Each task uses one existing queue worker as coordinator and bounded concurrent file workers as described below. The compact in-memory plan stores only message IDs, album IDs and video flags, capped at 100,000 associated source messages (including non-video album members). Exceeding this cap fails before any download. No plan is persisted to disk. Planned groups are reloaded before saving; deleted members or changed albums count as failures, while captions are not re-matched and filenames/rules use current message information. There are no new status databases, cursor files or persistent manifests. Ordinary download temporary files retain their existing cleanup on completion, error and cancellation. Existing logs, OS swap and Telegram session handling remain separate from collection state.
...
The existing `workers` setting also limits concurrent files within a collection. With `workers=2`, one collection can process two videos concurrently; all collections share two file slots. Ordinary save tasks retain their existing queue and do not share this collection-specific file limit. Saving applies backpressure. In addition to the bounded identifier-only plan, only a page, an album, bounded active items, counters and the latest error are held in memory. No new configuration or persistent state is introduced.
...
The progress layout is unchanged. Its heading distinguishes scanning, scan complete/saving, all saved successfully, completion with existing-name skips, no matches, failure and cancellation. A failed or canceled task never reports complete success.
